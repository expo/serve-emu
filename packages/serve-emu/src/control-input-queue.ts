...
export class ControlInputQueue {
  ...
  enqueue(gesture: Gesture, screen: Screen): ControlInputHandle {
    ...
    const moveKey =
      gesture.type === "touch" && gesture.action === "move"
        ? `touch:${gesture.pointerId ?? 0}`
        : null;
    const pointerKey =
      gesture.type === "touch" ? `touch:${gesture.pointerId ?? 0}` : null;
    let reservedReleases = this.#openPointers.size;
    if (
      pointerKey &&
      gesture.type === "touch" &&
      gesture.action === "down" &&
    ...
    if (moveKey && tail?.moveKey === moveKey) {
      ...
      return { gesture, completion: waiter.promise };
    }

    this.#reserveNew(compiled.bytes, reservedReleases);
    if (pointerKey && gesture.type === "touch") {
      if (gesture.action === "down") this.#openPointers.add(pointerKey);
      if (gesture.action === "up") this.#openPointers.delete(pointerKey);
    }
    this.#pending.push({
  ...
  enqueuePacket(
    ...
    if (coalesceKey && tail?.coalesceKey === coalesceKey) {
      this.#reserveCoalesced(tail, packet.length);
      const previous = tail.waiters.at(-1);
      if (previous) previous.status = "coalesced";
      tail.steps = [{ delayMs: 0, packet: Buffer.from(packet) }];
      tail.bytes = packet.length;
      tail.waiters.push(waiter.waiter);
      return { completion: waiter.promise };
    }

    this.#reserveNew(packet.length);
    this.#pending.push({
  ...
  #reserveNew(bytes: number, reservedReleases = this.#openPointers.size): void {
    this.#assertCapacity(this.#depth + 1, this.#bytes + bytes, reservedReleases);
    this.#depth++;
    this.#bytes += bytes;
  }
  ...
  #reserveCoalesced(entry: QueueEntry, bytes: number): void {
    ...
    this.#assertCapacity(this.#depth + 1, nextBytes, this.#openPointers.size);
    this.#depth++;
    this.#bytes = nextBytes;
  }

  #assertCapacity(depth: number, bytes: number, reservedReleases: number): void {
    if (
