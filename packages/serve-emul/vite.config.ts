...
function parseBackendOrigin(value: string | undefined): URL {
  ...
  try {
    ...
    throw new Error(
      "SERVE_EMUL_BACKEND_ORIGIN must be a valid HTTP or HTTPS origin",
    );
  }

  if (backend.protocol !== "http:" && backend.protocol !== "https:") {
    throw new Error("SERVE_EMUL_BACKEND_ORIGIN must use http:// or https://");
  }
  if (backend.username || backend.password) {
    throw new Error("SERVE_EMUL_BACKEND_ORIGIN must not include credentials");
  }
  if (backend.pathname !== "/") {
    throw new Error("SERVE_EMUL_BACKEND_ORIGIN must not include a path");
  }
  if (configuredOrigin.includes("?") || configuredOrigin.includes("#")) {
    throw new Error("SERVE_EMUL_BACKEND_ORIGIN must not include a query or hash");
  }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: createBackendProxy(env.SERVE_EMUL_BACKEND_ORIGIN),
    },
  };
});
