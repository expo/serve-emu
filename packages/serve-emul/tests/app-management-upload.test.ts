...
function upload(
  ...
  return {
    path: "/private/tmp/serve-emul-upload/upload",
    filename: "sample.apk",
...
describe("staged app management uploads", () => {
  ...
  test("imports through a hidden partial path before the final atomic rename", async () => {
    ...
    const partial =
      "/sdcard/Pictures/.serve-emul-fixed-My-photo.jpg.part";
  ...
  test("removes a remote partial file after an ADB failure", async () => {
    ...
    expect(calls.at(-1)?.args.slice(2)).toEqual([
      ...
      "rm",
      "-f",
      "/sdcard/Movies/.serve-emul-failed-clip.mp4.part",
    ]);
  ...
  test.each([".", "..", "/"])(
    ...
    async (filename) => {
      ...
      expect(calls[2]?.args.slice(2)).toEqual([
        ...
        "mv",
        "-f",
        "/sdcard/Download/.serve-emul-safe-upload-safe.part",
        final,
      ]);
    },
  );
  ...
  test("reports a remote partial cleanup failure with the primary failure", async () => {
    ...
    expect(error).toMatchObject({
      ...
      message:
        "failed to remove partial upload /sdcard/Download/.serve-emul-cleanup-large.bin.part",
    });
