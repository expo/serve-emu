...
const PACKAGE_LINK_REWRITES = [
  ["packages/serve-emul/package.json", "package.json"],
  ["packages/serve-emul/CHANGELOG.md", "CHANGELOG.md"],
  ["packages/serve-emul/docs/protocol.md", "docs/protocol.md"],
  [
    "CONTRIBUTING.md",
    "https://github.com/jiunshinn/serve-emul/blob/main/CONTRIBUTING.md",
  ],
] as const;
...
const PACKAGE_TEXT_REWRITES = [
  ["`packages/serve-emul/vendor/`", "`vendor/`"],
] as const;
...
async function main(): Promise<void> {
  ...
  if (args[0] === "--write") {
    ...
    console.log("Updated packages/serve-emul/README.md from README.md");
    return;
  }
  ...
  if (!checkPackageReadme(rootReadme, packageReadme).current) {
    console.error(
      "packages/serve-emul/README.md is stale; run `bun run scripts/sync-readme.ts --write` from packages/serve-emul",
    );
    process.exitCode = 1;
    return;
  }
